...
describe('compileWorkers.ts', () => {
  ...
  it('validate checking the workers', () => {
    ...
    if ('win32' === process.platform) {
      sinon.assert.calledWithExactly(existsSync.getCall(0), 'input1\\backend\\workers\\tsconfig.json');
    }
    else {
      sinon.assert.calledWithExactly(existsSync.getCall(0), 'input1/backend/workers/tsconfig.json');
    }
    ...
    if ('win32' === process.platform) {
      sinon.assert.calledWithExactly(existsSync.getCall(1), 'input2\\backend\\workers\\tsconfig.json');
      ...
      sinon.assert.calledWithExactly(existsSync.getCall(1), 'input2/backend/workers/tsconfig.json');
  ...
  it('validate compiling the workers', () => {
    ...
    readdirSync.onCall(0).returns([]);
    compile({ name: 'pkg' }, { name: 'cfg' }, 'input', 'output');
    if ('win32' === process.platform) {
      sinon.assert.calledWithExactly(readdirSync.getCall(0), 'input\\backend\\workers', { withFileTypes: true, recursive: true });
    }
    else {
      sinon.assert.calledWithExactly(readdirSync.getCall(0), 'input/backend/workers', { withFileTypes: true, recursive: true });
    }
    ...
    readdirSync.onCall(1).returns([{ name: 'test.tsx' }, { name: 'test.js' }]);
    compile({ name: 'pkg' }, { name: 'cfg' }, 'input', 'output');
    if ('win32' === process.platform) {
      sinon.assert.calledWithExactly(readdirSync.getCall(1), 'input\\backend\\workers', { withFileTypes: true, recursive: true });
    }
    else {
      sinon.assert.calledWithExactly(readdirSync.getCall(1), 'input/backend/workers', { withFileTypes: true, recursive: true });
    }
    ...
    readdirSync.onCall(2).returns([{ name: 'test1.ts', parentPath: 'path1' }, { name: 'test2.ts', parentPath: 'path2' }]);
    readFile.onCall(0).returns('').onCall(1).returns('');
    compile({ name: 'pkg' }, { name: 'cfg', version: '1.0.0' }, 'input', 'output');
    if ('win32' === process.platform) {
      sinon.assert.calledWithExactly(readdirSync.getCall(2), 'input\\backend\\workers', { withFileTypes: true, recursive: true });
    ...
    readdirSync.onCall(3).returns([{ name: 'test1.ts', parentPath: 'path1' }, { name: 'test2.ts', parentPath: 'path2' }]);
    readFile.onCall(2).returns('').onCall(3).returns('');
    compile({ name: 'pkg', dependencies: { a: 'b', b: 'c' } }, { name: 'cfg', version: '1.0.0' }, 'input', 'output');
    if ('win32' === process.platform) {
      sinon.assert.calledWithExactly(readdirSync.getCall(3), 'input\\backend\\workers', { withFileTypes: true, recursive: true });
    ...
    readdirSync.onCall(4).returns([{ name: 'test1.ts', parentPath: 'path1' }, { name: 'test2.ts', parentPath: 'path2' }]);
    readFile.onCall(4).returns('"use strict";\n"glc task @test"\nconsole.log("done")');
    compile({ name: 'pkg', dependencies: { a: 'b', b: 'c' } }, { name: 'cfg', version: '1.0.0' }, 'input', 'output');
    if ('win32' === process.platform) {
      sinon.assert.calledWithExactly(readdirSync.getCall(4), 'input\\backend\\workers', { withFileTypes: true, recursive: true });
    ...
    // Validating all regexes are not part of this test, it will be done in another test
    readdirSync.onCall(5).returns([{ name: 'test1.ts', parentPath: 'path1' }, { name: 'test2.ts', parentPath: 'path2' }]);
    readFile.onCall(5).returns('"use strict";\n"glc task @yearly"\nconsole.log("done")').onCall(6).returns('"use strict";\n"glc service @annually"\nconsole.log("done")');
    compile({ name: 'pkg', dependencies: { a: 'b', b: 'c' } }, { name: 'cfg', version: '1.0.0' }, 'input', 'output');
    if ('win32' === process.platform) {
      sinon.assert.calledWithExactly(readdirSync.getCall(5), 'input\\backend\\workers', { withFileTypes: true, recursive: true });
